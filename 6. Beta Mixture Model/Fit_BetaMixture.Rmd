...
params:
  ...
  outdir: 'Beta Peak Decomposition',
...
betaMixture_TCGA <- tryCatch(readRDS(file.path(consts['TCGA_datadir'],
...
if (is.null(betaMixture_TCGA)) {
  TCGA_clinical <- readRDS(file.path(consts['TCGA_datadir'],
                                  paste0(params$clinical_filename, ".rds")))
  
  analysis_tumors <- TCGA_clinical$in_analysis_dataset
  
  brca.methyl <- readRDS(file.path(consts['TCGA_datadir'],
                                paste0(params$methyl_filename, ".rds")))
  ...
  saveRDS(list(alpha = alpha, delta = delta, Z = z), 
          file = file.path(consts['TCGA_datadir'], 'Beta Peak Decomposition', 'TCGA',
                        "tcga_beta_mixture_model.rds"))
...
```{r get_lund_data}
lund.methyl <- read.csv(file.path(consts['Lund_datadir'], 
...
betaMixture_Lund<- tryCatch(readRDS(file.path(consts['Lund_datadir'],
...
if (is.null(betaMixture_Lund)) {
  ...
  saveRDS(list(alpha = alpha, delta = delta, Z = z), 
          file = file.path(consts['TCGA_datadir'], 'Beta Peak Decomposition', 'Lund'
                        "lund_beta_mixture_model.rds"))
