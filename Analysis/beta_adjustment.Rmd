...
params:
  data_dir: "../data/"
  methyl_filename: "rds object containing methylation data" 
  gene_filename: "rds object containing gene expression data"
  purity_filename: "csv file containing purity estimates"
  lookup_filename: "xlsx file containing CpG site specific beta values from WB"
  cpg_filename: "csv file containing clock set fCpG"
  cibersort_filename: "Cibersort_LM22_500perm_All_Tumors.csv"
editor_options: 
  markdown: 
    wrap: 72
---
...
```{r read_wbc_xlsx}
lookup <- readxl::read_excel(file.path(params$data_dir, params$lookup_filename))
...
gg <- ggplot(data = data_adj1, aes(purity, c_beta)) +
  ...
  ggpubr::stat_cor() +
  gg_theme +
  ggtitle(expression("Purity-Adjusted c"[beta]^alpha~"vs CPE")) +
...
gg <- ggplot(data = data_c_beta, aes(measured, adj1)) +
  ...
  ggpubr::stat_cor() +
  gg_theme +
  ggtitle(expression("Purity-Adjusted c"[beta]^alpha~"vs Measured c"[beta])) +
...
absolute_score_figure <- function(data, x, xname) {
  ...
  title <- parse(text = paste('"Absolute Score vs ', xname))
